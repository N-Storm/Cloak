...
for file in ck-client-*; do newname="$(echo \"${file}\" | sed -E 's/\.exe$/-noCGO.exe/')"; mv ${file} ${newname} ; done
...
for file in ck-server-*; do newname="$(echo \"${file}\" | sed -E 's/\.exe$/-noCGO.exe/')"; mv ${file} ${newname} ; done
